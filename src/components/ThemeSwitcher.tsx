...
const ThemeSwitcher = () => {
  ...
  return <div className="fixed top-20 right-6 z-50">
      ...
      {isOpen && <div className="absolute top-full right-0 mt-2 w-72 rounded-lg overflow-hidden animate-fade-in backdrop-blur-md shadow-xl border" style={{
      ...
      borderColor: themes[currentTheme as keyof typeof themes].border
        ...
        borderBottomColor: themes[currentTheme as keyof typeof themes].border
            ...
            <h3 className="font-mono font-semibold" style={{
          color: themes[currentTheme as keyof typeof themes].primary
        }}>
              Choose Theme
            </h3>
          ...
          <div className="p-2 space-y-1">
            {Object.entries(themes).map(([key, theme]) => <button key={key} onClick={() => handleThemeSelect(key)} className="w-full p-3 rounded-lg transition-all duration-200 text-left group" style={{
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className="flex space-x-1">
                      <div className="w-3 h-3 rounded-full border border-opacity-30 shadow-sm" style={{
                  ...
                  borderColor: `${themes[currentTheme as keyof typeof themes].border}30`
                    ...
                    <div>
                      <div className="font-mono text-sm font-medium" style={{
                  color: themes[currentTheme as keyof typeof themes].text
                }}>
                        {theme.name}
                      </div>
                      <div className="text-xs opacity-70" style={{
                  color: themes[currentTheme as keyof typeof themes].text
                }}>
                        {theme.description}
                      </div>
                  ...
                  {currentTheme === key && <Check className="w-4 h-4 animate-fade-in" style={{
              color: themes[currentTheme as keyof typeof themes].primary
            }} />}
                </div>
              </button>)}
